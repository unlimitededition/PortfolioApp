...
//	Debug.Log("current position " + currentPos);
	...
	{	
		...
		//Debug.Log("Click Down " + clickDown);
		//Debug.Log("Click Up " + clickUp);
		
		if (clickPos != (Screen.width/2))
		{
    		sliding = true;
    	}
    	
		if (clickPos.x<(Screen.width/2) && currentSlide<0 && sliding)		//This was a flick to the right with magnitude of 5 or more
		{
			currentSlide ++;
			sliding = false;
		}
		
		if (clickPos.x>(Screen.width/2) && currentSlide>(-1*(visual.Length-1)) && sliding)	//This was a flick to the left with magnitude of 5 or more
		{
			currentSlide --;
			sliding = false;
		}
